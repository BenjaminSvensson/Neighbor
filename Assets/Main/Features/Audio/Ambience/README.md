...
4. Add `AmbienceArea` and a collider to each area, then assign its profile and choose whether its `Zone Location` is outside, inside, basement, or garage. The collider is automatically made a trigger.
...
Drop a prefab into the scene, resize its trigger collider to match the space, assign the area's `AmbienceProfile`, and set `Zone Location` to the matching space. While the player listener is inside that collider, the assigned ambience profile plays.
