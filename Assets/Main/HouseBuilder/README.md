...
3. Use **Place** and click an asset card. Move into Scene view and left-click to place copies. Click the selected green card again or press `Esc` to stop placement. Right-click or hold and drag right-click over matching placed objects to erase them. Use `Q`/`E` for yaw, `R`/`F` for pitch, and `Z`/`C` for roll.
